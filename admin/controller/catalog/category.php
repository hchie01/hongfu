...
class ControllerCatalogCategory extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if($pid!=0){
				$ppath = $this->model_catalog_category->getCatePath($pid)['path'];
			}
			if($pid==0){
				$this->request->post['path'] = $pid.',';
			}else{
				$this->request->post['path'] = $ppath.$pid.',';
			}
...
// echo '</pre>';die;
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if($pid==0){
				$this->request->post['path'] = (string)$pid.',';
			}else{
				$cd = $this->request->post['category_description'];
				foreach($cd as $k=>$v){
					$this->request->post['category_description'][$k]['path'] = $ppath.$pid.',';
				}
			}
	...
	protected function getList() {
		...
		$results = $this->model_catalog_category->getCates($filter_data);
	...
	protected function getForm() {
		...
		$this->load->model('catalog/category');
		$data['cates'] = $this->model_catalog_category->getCates();//$data['cates']
		...
		// 	$crr[$k]['name'] = str_repeat('&nbsp;',$n*4).$str.$crr[$k]['name'];
		// }
