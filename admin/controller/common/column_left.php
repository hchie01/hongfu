...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ((string)$this->request->get['user_token'] == $this->session->data['user_token'])) {
            ...
            //左侧菜单新增  板块管理
            if ($this->user->hasPermission('access', 'catalog/blocklist')) {
                $catalog[] = array(
                    'name'	   => $this->language->get('text_blocklist'),
                    'href'     => $this->url->link('catalog/blocklist', 'user_token=' . $this->session->data['user_token']),
                    'children' => array()
                );
            }
