<?php
class ControllerCatalogEbaypro extends Controller {
	private $error = array();
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            if (isset($this->request->get['filter_fuprice'])) {
                $url .= '&filter_fuprice=' . $this->request->get['filter_fuprice'];
            }
...
//            $ebaypath = ($this->request->get['route'] === 'catalog/ebaypro/add')? 'catalog/ebaypro' : 'catalog/product';
			$this->response->redirect($this->url->link('catalog/ebaypro', 'user_tokens=' . $this->session->data['user_tokens'] . $url));
		}
		$this->getForm();
	}
	...
	protected function getList() {
        ...
        if (isset($this->request->get['filter_fuprice'])) {
            $filter_fuprice = $this->request->get['filter_fuprice'];
        } else {
            $filter_fuprice = '';
        }
...
//
        foreach ($results as $result) {
			...
			foreach ($product_specials  as $product_special) {
				if (($product_special['date_start'] == '0000-00-00' || strtotime($product_special['date_start']) < time()) && ($product_special['date_end'] == '0000-00-00' || strtotime($product_special['date_end']) > time())) {
                    ...
                    $special = $this->currency->format($product_special['fuprice'], $this->config->get('config_currency'));
			...
			$data['products'][] = array(
                ...
                'fuprice'    => $this->currency->format($result['fuprice'], $this->config->get('config_currency')),
				'special'    => $special,
				'quantity'   => $result['quantity'],
				'status'     => $result['status'],
				'view'       => $this->front_url->link('ebaypro/ebaypro', "product_id={$result['product_id']}"),
		...
		$pagination->url = $this->url->link('catalog/ebaypro', 'user_tokens=' . $this->session->data['user_tokens'] . $url . '&page={page}');
	...
	protected function getForm() {
        ...
        if (isset($this->request->post['fuprice'])) {
            $data['fuprice'] = $this->request->post['fuprice'];
        } elseif (!empty($product_info)) {
            $data['fuprice'] = $product_info['fuprice'];
        } else {
            $data['fuprice'] = '';
        }
