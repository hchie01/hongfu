...
class ModelCatalogProduct extends Model {
	...
	//简单的查询出商品的名称价格
	public function getEasys($num='')
	{
		$sql = "select p.product_id,p.shop_id,p.image,p.price,pd.name from  " . DB_PREFIX . "product p LEFT JOIN ". DB_PREFIX . "product_description pd on (p.product_id=pd.product_id) where pd.language_id = 2 ";
		...
		foreach ($query->rows as $k=>$v) {
			$query->rows[$k]['image'] = 'image/'.$v['image'];
		}
